...
namespace HELPSTEIN
{
    class TeamViewer
    {
        public string tvFullText = "";
        DateTime dataTime = DateTime.Now;
        public string DataTableTeamViewerQuery = "";
        ...
        public string get_TeamViewer_id_pass() //string argDelim = constSmallDelim, string argErrResult = "Не найден процесс TV!")
        {
           ...
           // teamViewerQuery += @"INSERT INTO public.team_viewer (id, date, full_text, id_team_viewer, password)  VALUES ";
            ...
            if (procParentHandle != IntPtr.Zero)
            {
                ...
                foreach (var item in teamviewerText)
                {
                    ...
                    if (isNextLineIsTVPass)
                    {
                        ...
                        tvFullText += resultAll;
                        // result += resultAll;
                        DataTableTeamViewerQuery += currentline;
                        if (tvFullText.Length > 256) { tvFullText = tvFullText.Remove(tvFullText.Length - (tvFullText.Length - 256)); }
                        DataTableTeamViewerQuery += "," + tvFullText.Trim() + ";";
                        return result;
                    }
                    ...
                    if (m.Success)
                    {
                        ...
                        DataTableTeamViewerQuery += ids.ids + "," + dataTime.ToString("HH:mm:ss dd.MM.yyyy") + "," + currentline + ",";
                        //return result;
                    }
            ...
            if (result.StartsWith(argErrResult)) { DataTableTeamViewerQuery += ids.ids + "," + dataTime.ToString("HH:mm:ss dd.MM.yyyy") + "," + argErrResult + "," + argErrResult + "," + argErrResult + "," + argErrResult + ";"; }
